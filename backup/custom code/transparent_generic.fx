	...
	in float4 misc)
{
	...
	float3 T3 = shield_mask.rgb;

	float T3a = shield_mask.a;
	float T2a = shield_offset.a;
	float T1a = plasma_noise_b.a;
	float T0a = plasma_noise_a.a;

	*/

// Initial input (Stage 0) and some explanation as to how it works in Combat Evolved
	/*
	...
	whilc output abcd mux returns cd when scratch color 0 is greater than 0.5 and returns ab otherwise

	*/

	float3 vertex_color_1 = dot(view_dir, normal);

	float3	r0_input = SUM(INVERT(plasma_factor1), shield_offset.a, plasma_factor1, plasma_noise_a.a);
	float	r0a_input = SUM(INVERT(plasma_factor2), 0.5, plasma_factor2, plasma_noise_b.a);

// Plasma (Stage 1)
	float3	r0_plasma = SUM(shield_mask.a, 0.5, INVERT(shield_mask.a), r0_input);
	float	r0a_plasma = SUM(shield_mask.a, 0.5, INVERT(shield_mask.a), r0a_input);

// Half-bias (Stage 2)
	float3	r0_hb = r0_plasma + HALF_BIAS_NEGATIVE(r0a_plasma);
	float	r0a_hb = r0a_plasma + HALF_BIAS_NEGATIVE(r0_plasma.b);

// Plasma Blend and Scale by 4 stage (Stage 3)
	float3	r0_plasma_blend = SUM(INVERT(plasma_color.a), plasma_color.rgb, plasma_color.a, plasma_flash_color) * vertex_color_1;
	float	r0a_4 = SCALE4(MUX2(r0a_hb, r0a_hb, r0a_hb, r0_hb.b, r0_hb.b));

// Sharpening time (Stage 4)
	float	r0a_sharp = MUX2(r0a_4, 0, 0, EXPAND(r0a_4), EXPAND(r0a_4));

// Dulling
	float	r0a_dull = SUM(r0a_sharp, 1, r0a_sharp, INVERT(r0a_sharp));

// Final Output
	albedo.rgb = SUM(r0a_dull, shield_mask.rgb, r0_plasma_blend, INVERT(shield_mask.a)) * plasma_brightness;
	albedo.a = r0a_dull;

}

void calc_albedo_plasma_offset_no_cc_ps(
	in float2 texcoord,
	out float4 albedo,
    in float3 normal,
	in float4 misc)
...
// Plasma Blend and Scale by 4 stage (Stage 3)
	float3	r0_plasma_blend = SUM_ABCD(INVERT(plasma_color.a), plasma_color.rgb, plasma_color.a, plasma_flash_color);
	float	r0a_4 = SCALE4((r0a_hb>0.5 ? r0_hb.b * r0_hb.b : r0a_hb * r0a_hb));
...
// Final Output
	albedo.rgb = SUM_ABCD(r0a_dull, shield_mask.rgb, r0_plasma_blend, INVERT(shield_mask.a)) * plasma_brightness;
	albedo.a = r0a_dull;
